    ...
    private function insertarDatosDespachosExternos($spreadsheet, $filtros = []) {
        ...
        try {
            ...
            foreach ($despachos as &$row) {
                ...
                // Destino: la consulta del modelo ya devuelve el nombre resuelto en 'Destino'
                // (COALESCE(dest.Empresa, '')) y el valor crudo de despachos_externos en
                // 'DestinoOriginal'. Se prioriza el nombre resuelto; si está vacío, se usa
                // el campo literal Destino de la tabla (sin concatenar RUC/Dirección).
                $destResuelto = trim((string)($row['Destino'] ?? ''));
                if ($destResuelto !== '') {
                    $row['Destino'] = $destResuelto;
                    $row['DestinoTexto'] = $destResuelto;
                } else {
                    $destOriginal = isset($row['DestinoOriginal']) && $row['DestinoOriginal'] !== null && $row['DestinoOriginal'] !== ''
                        ? (string)$row['DestinoOriginal']
                        : (isset($row['destino']) ? (string)$row['destino'] : '');
                    $row['Destino'] = $destOriginal;
                    $row['DestinoTexto'] = $destOriginal;
                }
        ...
        $cabeceras = [
            'A1' => 'NRO VALE', 'B1' => 'FECHA', 'C1' => 'HORA', 'D1' => 'TURNO', 'E1' => 'DESPACHADOR',
            'F1' => 'DESTINO', 'G1' => 'RUC', 'H1' => 'DIRECCIÓN', 'I1' => 'CHOFER', 'J1' => 'BREVETE',
            'K1' => 'TRANSPORTISTA', 'L1' => 'RUC TRANSPORTISTA', 'M1' => 'PLACA TRACTO', 'N1' => 'CONSTANCIA INSC. TRACTO',
            'O1' => 'PLACA CARRETA', 'P1' => 'CONSTANCIA INSC. CARRETA', 'Q1' => 'GUÍA REMISIÓN',
            'R1' => 'CÓDIGO', 'S1' => 'PRODUCTO', 'T1' => 'UNIDAD MEDIDA', 'U1' => 'CANTIDAD', 'V1' => 'COMENTARIOS', 'W1' => 'ESTADO'
        ];
            ...
            foreach ($despachos as $despacho) {
                ...
                if (count($productos) > 0) {
                    foreach ($productos as $producto) {
                        ...
                        $sheet->setCellValue('R' . $filaExcel, $producto['Codigo'] ?? '');
                        // La descripción puede venir con el prefijo "CODIGO - " (el formulario
                        // usaba el texto completo del select). Se limpia para mostrar solo la
                        // descripción (campo DescripcionProducto) sin duplicar el código.
                        $descProducto = isset($producto['Producto']) ? (string)$producto['Producto'] : '';
                        $codigoProducto = isset($producto['Codigo']) ? (string)$producto['Codigo'] : '';
                        if ($descProducto !== '' && $codigoProducto !== '') {
                            $prefijo = $codigoProducto . ' - ';
                            if (stripos($descProducto, $prefijo) === 0) {
                                $descProducto = trim(substr($descProducto, strlen($prefijo)));
                            }
                        }
                        $sheet->setCellValue('S' . $filaExcel, $descProducto);
                        $sheet->setCellValue('T' . $filaExcel, $producto['UnidadMedida'] ?? '');
                        $sheet->setCellValue('U' . $filaExcel, $producto['Cantidad'] ?? '');
                        $sheet->setCellValue('V' . $filaExcel, $producto['Comentarios'] ?? '');
                        $sheet->setCellValue('W' . $filaExcel, $despacho['Estado'] ?? '');
                        ...
                        if (isset($guia['Productos']) && is_array($guia['Productos']) && count($guia['Productos']) > 0) {
                            foreach ($guia['Productos'] as $producto) {
                                ...
                                $totalNeto = isset($producto['Total']) && $producto['Total'] !== '' && $producto['Total'] !== null
                                    ? $producto['Total']
                                    : ($producto['Cantidad'] ?? '');
                                $sheet->setCellValue('J' . $filaExcel, $totalNeto);
                                $filaExcel++;
                            }
                        } else {
                            $sheet->setCellValue('A' . $filaExcel, $recepcion['Turno'] ?? '');
