<div class="container-fluid px-0">
    <div class="d-flex align-items-center mb-4">
        <i class="bi bi-people display-6 text-primary me-2"></i>
        <h2 class="mb-0 fw-bold" style="letter-spacing:0.5px; color:#1a237e;">Mantenimiento de Clientes Externos</h2>
    </div>
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body py-2 px-3">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 flex-nowrap">
                <a href="<?= app_url('clientesexternos/create') ?>" class="btn btn-success fw-bold px-3">
                    <i class="bi bi-plus-circle me-1"></i> Nuevo Cliente Externo
                </a>
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2 justify-content-end" style="flex:1 1 auto; min-width:0;">
                    <form class="d-flex align-items-center gap-1" method="get" action="<?= app_url('clientesexternos') ?>" style="min-width:0;">
                        <input type="text" name="busqueda" class="form-control form-control-sm" placeholder="Buscar cliente..." value="<?= htmlspecialchars($busqueda ?? '') ?>" style="width:240px; max-width:260px;">
                        <button type="submit" class="btn btn-primary btn-sm px-2 ms-2" title="Buscar"><i class="bi bi-search"></i></button>
                        <?php
 if (!empty($busqueda)): ?>
                            <a href="<?= app_url('clientesexternos') ?>" class="btn btn-secondary btn-sm px-2 ms-1" title="Limpiar"><i class="bi bi-x-lg"></i></a>
                        <?php endif; ?>
                        <?php
                            $exportUrl = app_url('clientesexternos/exportar' . (!empty($busqueda) ? ('?busqueda=' . urlencode($busqueda)) : ''));
                        ?>
                    ...
                    <nav aria-label="Paginación de clientes externos" class="ms-md-3" style="min-width:0;">
                        <ul class="pagination pagination-sm mb-0 flex-nowrap" style="gap:0.25rem; min-width:unset;">
                            <li class="page-item<?= ($pagina ?? 1) <= 1 ? ' disabled' : '' ?>">
                                <?php
                                    $prevPageUrl = app_url('clientesexternos?pagina=' . (($pagina ?? 1) - 1) . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                ?>
                            ...
                            if ($startPage > 1): ?>
                                <li class="page-item">
                                    <a class="page-link rounded-pill px-2" href="<?= app_url('clientesexternos?pagina=1' . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : '')) ?>">1</a>
                                </li>
                                <?php if ($startPage > 2): ?>
                            ...
                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                <li class="page-item<?= $currentPage == $i ? ' active' : '' ?>">
                                    <?php
                                        $pageUrl = app_url('clientesexternos?pagina=' . $i . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                    ?>
                            ...
                            <?php if ($endPage < $totalPaginas): ?>
                                ...
                                <?php endif; ?>
                                <li class="page-item">
                                    <a class="page-link rounded-pill px-2" href="<?= app_url('clientesexternos?pagina=' . $totalPaginas . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : '')) ?>"><?= $totalPaginas ?></a>
                                </li>
                            <?php endif; ?>
                            <li class="page-item<?= ($pagina ?? 1) >= $totalPaginas ? ' disabled' : '' ?>">
                                <?php
                                    $nextPageUrl = app_url('clientesexternos?pagina=' . (($pagina ?? 1) + 1) . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                ?>
                        ...
                        <?php if (!empty($clientes)) {
                            foreach ($clientes as $c) { ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($c['Direccion2'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($c['Direccion3'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($c['Direccion4'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($c['Direccion5'] ?? '') ?></td>
                                    <td class="text-center" style="white-space:nowrap; min-width:90px;">
                                        <div class="d-inline-flex align-items-center gap-1">
                                            <?php $editUrl = app_url('clientesexternos/edit/' . urlencode($c['Id'])); ?>
                                            <a href="<?= $editUrl ?>" class="btn btn-sm btn-outline-primary px-2" title="Editar"><i class="bi bi-pencil"></i></a>
                                            <button type="button" class="btn btn-sm btn-outline-danger px-2" data-bs-toggle="modal" data-bs-target="#modalEliminar<?= $c['Id'] ?>" title="Eliminar"><i class="bi bi-trash"></i></button>
                                        </div>
                                        <!-- Modal -->
                                        <div class="modal fade" id="modalEliminar<?= $c['Id'] ?>" tabindex="-1" aria-labelledby="modalLabel<?= $c['Id'] ?>" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content border-0 shadow-lg rounded-4">
                                                    <div class="modal-header bg-danger bg-opacity-10 border-0 rounded-top-4">
                                                        ...
                                                        <h5 class="modal-title fw-bold text-danger" id="modalLabel<?= $c['Id'] ?>">Confirmar eliminación</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                                                    </div>
                                                    <div class="modal-body text-center">
                                                        <p class="mb-2">¿Seguro que deseas eliminar el cliente externo<br><strong><?= htmlspecialchars($c['Empresa']) ?></strong>?</p>
                                                    ...
                                                    <div class="modal-footer border-0 d-flex justify-content-between gap-2 pb-3">
                                                        ...
                                                        <?php $deleteUrl = app_url('clientesexternos/delete/' . urlencode($c['Id'])); ?>
                            ...
                            <tr><td colspan="9" class="text-center text-muted">No se encontraron clientes externos.</td></tr>
                        <?php } ?>
                    </tbody>
                </table>
            </div>
