<div class="container-fluid px-0">
    <div class="d-flex align-items-center mb-4">
        ...
        <h2 class="mb-0 fw-bold" style="letter-spacing:0.5px; color:#1a237e;">Mantenimiento de Series</h2>
    </div>
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body py-2 px-3">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 flex-nowrap">
                <a href="<?= app_url('series/create') ?>" class="btn btn-success fw-bold px-3">
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2 justify-content-end" style="flex:1 1 auto; min-width:0;">
                    <form class="d-flex align-items-center gap-1" method="get" action="<?= app_url('series') ?>" style="min-width:0;">
                        <input type="text" name="busqueda" class="form-control form-control-sm" placeholder="Buscar serie..." value="<?= htmlspecialchars($search ?? '') ?>" style="width:240px; max-width:260px;">
                        <button type="submit" class="btn btn-primary btn-sm px-2 ms-2" title="Buscar"><i class="bi bi-search"></i></button>
                        <?php
                        if (!empty($search)):
                        ?>
                            <a href="<?= app_url('series') ?>" class="btn btn-secondary btn-sm px-2 ms-1" title="Limpiar"><i class="bi bi-x-lg"></i></a>
                        <?php endif; ?>
                        <?php
                            $exportUrl = app_url('series/exportar' . (!empty($search) ? ('?busqueda=' . urlencode($search)) : ''));
                        ?>
                    ...
                    <nav aria-label="Paginación de series" class="ms-md-3" style="min-width:0;">
                        <ul class="pagination pagination-sm mb-0 flex-nowrap" style="gap:0.25rem; min-width:unset;">
                            <li class="page-item<?= ($page ?? 1) <= 1 ? ' disabled' : '' ?>">
                                <?php
                                    $prevPageUrl = app_url('series?pagina=' . (($page ?? 1) - 1) . (!empty($search) ? ('&busqueda=' . urlencode($search)) : ''));
                                ?>
                            ...
                            if ($startPage > 1): ?>
                                <li class="page-item">
                                    <a class="page-link rounded-pill px-2" href="<?= app_url('series?pagina=1' . (!empty($search) ? ('&busqueda=' . urlencode($search)) : '')) ?>">1</a>
                                </li>
                                <?php if ($startPage > 2): ?>
                            ...
                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                <li class="page-item<?= $currentPage == $i ? ' active' : '' ?>">
                                    <?php
                                        $pageUrl = app_url('series?pagina=' . $i . (!empty($search) ? ('&busqueda=' . urlencode($search)) : ''));
                                    ?>
                            ...
                            <?php if ($endPage < $totalPaginas): ?>
                                ...
                                <?php endif; ?>
                                <li class="page-item">
                                    <a class="page-link rounded-pill px-2" href="<?= app_url('series?pagina=' . $totalPaginas . (!empty($search) ? ('&busqueda=' . urlencode($search)) : '')) ?>"><?= $totalPaginas ?></a>
                                </li>
                            <?php endif; ?>
                            <li class="page-item<?= ($page ?? 1) >= $totalPaginas ? ' disabled' : '' ?>">
                                <?php
                                    $nextPageUrl = app_url('series?pagina=' . (($page ?? 1) + 1) . (!empty($search) ? ('&busqueda=' . urlencode($search)) : ''));
                                ?>
                        ...
                        <?php if (!empty($series)) {
                            foreach ($series as $s) { ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($s['Serie']) ?></td>
                                    <td><?= htmlspecialchars($s['CentroDistribucion']) ?></td>
                                    <td class="text-center" style="white-space:nowrap; min-width:90px;">
                                        <div class="d-inline-flex align-items-center gap-1">
                                            <?php $editUrl = app_url('series/edit/' . urlencode($s['Id'])); ?>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content border-0 shadow-lg rounded-4">
                                                    ...
                                                    <div class="modal-body text-center">
                                                        <p class="mb-2">¿Seguro que deseas eliminar la serie<br><strong><?= htmlspecialchars($s['Serie']) ?></strong>?</p>
                                                        <p class="text-muted small mb-0">Esta acción no se puede deshacer.</p>
                                                    </div>
                                                    <div class="modal-footer border-0 justify-content-center">
                                                        <button type="button" class="btn btn-secondary px-4" data-bs-dismiss="modal">Cancelar</button>
                                                        <?php $deleteUrl = app_url('series/delete/' . urlencode($s['Id'])); ?>
                                                        <a href="<?= $deleteUrl ?>" class="btn btn-danger px-4">Eliminar</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
