<div class="container-fluid px-0" style="padding-bottom:0;">
    <div class="d-flex align-items-center mb-4">
        ...
        <h2 class="mb-0 fw-bold" style="letter-spacing:0.5px; color:#1a237e;">Mantenimiento de Usuarios</h2>
    </div>
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body py-2 px-3">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 flex-nowrap">
                <a href="<?= app_url('usuarios/create') ?>" class="btn btn-success fw-bold px-3">
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2 justify-content-end" style="flex:1 1 auto; min-width:0;">
                    <form class="d-flex align-items-center gap-1" method="get" action="<?= app_url('usuarios') ?>" style="min-width:0;">
                        ...
                        <?php
 if (!empty($busqueda)): ?>
                            <a href="<?= app_url('usuarios') ?>" class="btn btn-secondary btn-sm px-2 ms-1" title="Limpiar"><i class="bi bi-x-lg"></i></a>
                        <?php endif; ?>
                        <?php
                            $exportUrl = app_url('usuarios/exportar' . (!empty($busqueda) ? ('?busqueda=' . urlencode($busqueda)) : ''));
                        ?>
                    ...
                    <nav aria-label="Paginación de usuarios" class="ms-md-3" style="min-width:0;">
                        <ul class="pagination pagination-sm mb-0 flex-nowrap" style="gap:0.25rem; min-width:unset;">
                            <li class="page-item<?= ($pagina ?? 1) <= 1 ? ' disabled' : '' ?>">
                                <?php
                                    $prevPageUrl = app_url('usuarios?pagina=' . (($pagina ?? 1) - 1) . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                ?>
                            ...
                            <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                                <li class="page-item<?= ($pagina ?? 1) == $i ? ' active' : '' ?>">
                                    <?php
                                        $pageUrl = app_url('usuarios?pagina=' . $i . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                    ?>
                            ...
                            <li class="page-item<?= ($pagina ?? 1) >= $totalPaginas ? ' disabled' : '' ?>">
                                <?php
                                    $nextPageUrl = app_url('usuarios?pagina=' . (($pagina ?? 1) + 1) . (!empty($busqueda) ? ('&busqueda=' . urlencode($busqueda)) : ''));
                                ?>
                ...
                <table class="table table-hover align-middle mb-0" style="margin-bottom:0;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= htmlspecialchars($u['DocIdentidad']) ?></td>
                            <td><?= htmlspecialchars($u['NombresApellidos']) ?></td>
                            <td><?= htmlspecialchars($u['RolNombre'] ?? '') ?></td>
                            <td class="text-end">
                                <div class="d-inline-flex align-items-center gap-1">
                                    <?php $editUrl = app_url('usuarios/edit/' . $u['Id']); ?>
                                    <a href="<?= $editUrl ?>" class="btn btn-sm btn-outline-primary px-2" title="Editar"><i class="bi bi-pencil"></i></a>
                                    <button type="button" class="btn btn-sm btn-outline-danger px-2" data-bs-toggle="modal" data-bs-target="#modalEliminar<?= $u['Id'] ?>" title="Eliminar"><i class="bi bi-trash"></i></button>
                                </div>
                                <!-- Modal -->
                                <div class="modal fade" id="modalEliminar<?= $u['Id'] ?>" tabindex="-1" aria-labelledby="modalLabel<?= $u['Id'] ?>" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content border-0 shadow-lg rounded-4">
                                            <div class="modal-header bg-danger bg-opacity-10 border-0 rounded-top-4">
                                                ...
                                                <h5 class="modal-title fw-bold text-danger" id="modalLabel<?= $u['Id'] ?>">Confirmar eliminación</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                                            </div>
                                            <div class="modal-body text-center">
                                                <p class="mb-2">¿Seguro que deseas eliminar el usuario<br><strong><?= htmlspecialchars($u['username']) ?></strong>?</p>
                                            ...
                                            <div class="modal-footer border-0 d-flex justify-content-between gap-2 pb-3">
                                                ...
                                                <?php $deleteUrl = app_url('usuarios/delete/' . $u['Id']); ?>
