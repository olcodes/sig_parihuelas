    ...
    <div class="card shadow-sm border-0" style="width:100%; max-width:480px;">
        <div class="card-body p-4">
            <div class="mb-4 d-flex align-items-center">
                ...
                <div>
                    <h3 class="mb-0 fw-bold" style="letter-spacing:0.5px; color:#1a237e;">Editar Cliente Externo</h3>
                ...
                <div class="mb-3 d-flex justify-content-center">
                    <div style="width:100%; max-width:480px; margin:0 auto;">
                        ...
                        <div class="input-group input-group-lg">
                            <input type="text" class="form-control" id="RUC" name="RUC" maxlength="11" value="<?= htmlspecialchars($data['RUC'] ?? '') ?>" required>
...
(function(){
    ...
    try {
        ...
        if (manualToggle) {
            manualToggle.checked = false; // por defecto desmarcado
            ...
            // Direccion2..5 quedan habilitadas por defecto
    ...
    } catch(e) { console.warn('toggle manual error (edit clientesexternos)', e); }
    btn.addEventListener('click', async function (e) {
        ...
        try {
            // Primero: verificar en el servidor si ya existe el RUC (en edición enviamos excludeId)
            try {
                const excludeIdInput = document.querySelector('input[name="Id"]');
                const excludeId = excludeIdInput ? excludeIdInput.value : null;
                const chk = await fetch('<?= app_url("clientesexternos/checkRuc") ?>', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ ruc: ruc, excludeId: excludeId })
                });
                const chkText = await chk.text();
                let chkJson = null;
                try { chkJson = chkText ? JSON.parse(chkText) : null; } catch(e) { chkJson = { success: false }; }
                console.debug('checkRuc early response (edit):', chkJson);
            ...
            } catch (e) { console.warn('checkRuc early failed (edit)', e); }
