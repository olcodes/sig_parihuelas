...
document.addEventListener('DOMContentLoaded', function() {
    // Helper seguro para parsear respuestas que deberían ser JSON
    function parseJSONorRedirect(response) {
        return response.text().then(text => {
            try {
                ...
                console.error('[RecepcionesInternas] respuesta no JSON detectada:', snippet);
                if (snippet.indexOf('<!DOCTYPE') !== -1 || snippet.indexOf('Cerrar Sesión') !== -1) {
                    mostrarMensajeError('Sesión expirada o respuesta inesperada. Redirigiendo al login...', 'error');
    ...
    window.cargarRecepcionParaEditar = function(id) {
        fetch((window.APP_URL || window.BASE_URL) + '/recepcionesinternas/getRecepcion', {
            method: 'POST',
    ...
    if (btnModificar) {
        ...
        btnModificar.addEventListener('click', function() {
            ...
            let recepcionId = correlativoValeEl ? correlativoValeEl.getAttribute('data-id') : null;
            ...
            fetch((window.APP_URL || window.BASE_URL) + '/recepcionesinternas/verificarModificaciones?id=' + recepcionId)
                .then(parseJSONorRedirect)
                .then(data => {
                    if (data.success && data.puede_modificar) {
                        ...
                        const mensaje = data.mensaje || 'Esta recepción ya alcanzó el límite de 3 modificaciones';
                        mostrarMensajeError(mensaje, 'error');
                    }
                })
                .catch(error => {
                    console.error('[Recepciones Internas] Error al verificar modificaciones:', error);
            ...
            if (opt) {
                ...
                if (window.choicesInstances && window.choicesInstances['producto']) {
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            try { if (typeof window.updateAllChoicesWrap === 'function') window.updateAllChoicesWrap(); } catch(e){}
                        }, 50);
                    }, 0);
                }
            }
    ...
    window.modificarrecepción = function(id, data) {
        fetch((window.APP_URL || window.BASE_URL) + '/recepcionesinternas/modificar', {
            method: 'POST',
    ...
    if (btnImprimir) {
        btnImprimir.addEventListener('click', function(event) {
            ...
            const vale = {
                ...
                medioTransporte: getSelectTextOrEmpty('medioTransporte'),
            ...
            let html = `<div style='font-family:Arial,sans-serif; padding:10px; font-size:0.65rem; line-height:1.08;'>
                    ...
                    <h3 style='text-align:center; font-weight:bold; margin-top:6px; font-size:0.81rem; margin-bottom:4px;'>VALE DE RECEPCIÓN INTERNA</h3>
                        ...
                        <tr>
                            <td style='width:33%; vertical-align:bottom; box-sizing:border-box; padding-bottom:6px;'>
                                <div style='display:flex; flex-direction:column; align-items:center;'>
                                    <div style='border-top:0.25pt solid #000; width:80%; margin:15px 0 8px 0;'></div>
    ...
    fetch((window.APP_URL || window.BASE_URL) + '/recepcionesinternas/siguienteVale')
        .then(parseJSONorRedirect)
        .then(res => {
    ...
    if (btnNuevo) {
        btnNuevo.addEventListener('click', function() {
            // --- 1. LIMPIAR TODOS LOS VALORES DE LOS CONTROLES PRINCIPALES ---
            // limpiarControles() ya limpia producto, codigo, cantidad, comentarios
            // Adicionalmente limpiamos los campos del encabezado
            var camposAdicionales = ['subarea','emisor','despachador','medioTransporte','verificador','area'];
            ...
            // --- 2. LIMPIAR LA GRILLA DE PRODUCTOS ---
            var grillaBody = document.getElementById('grillaRecepcion')?.querySelector('tbody');
            if(grillaBody) {
                grillaBody.innerHTML = '';
            }
            // Resetear arreglo de datos de productos (del script de edición)
            try{ if(typeof datosGrilla !== 'undefined') datosGrilla = []; }catch(e){}
            // Resetear estado de edición
            try{ editIndex = null; }catch(e){}
            try{ updatePhase = null; }catch(e){}
            // Resetear valeIdActual
            try{ if(typeof valeIdActual !== 'undefined') valeIdActual = null; }catch(e){}
            try{ window.valeIdActual = null; }catch(e){}
            ...
            (function(){
                var url = (window.APP_URL || window.BASE_URL || '') + '/recepcionesinternas/siguienteVale';
                fetch(url, { credentials: 'same-origin' })
                    .then(function(r){ return r.json(); })
                    .then(function(res){
                        if(res && res.success && res.correlativo && correlativoVale){
                            var num = res.correlativo.toString().replace(/^VRI-/, '').padStart(6, '0');
                            correlativoVale.textContent = 'VRI-' + num;
                        }
                    })
                    .catch(function(err){ console.warn('[RecepcionesInternas] Error obteniendo siguiente correlativo:', err); });
            })();

            // --- 4. RESETEAR TURNO AUTOMÁTICO ---
            try{
                var chkTurnoEl = document.getElementById('chkTurno');
                if(chkTurnoEl) chkTurnoEl.checked = false;
                if(typeof setTurnoByHora === 'function') setTurnoByHora();
                // Aplicar fecha efectiva para turno nocturno
                try{ if(typeof window.aplicarFechaEfectivaTurno === 'function') window.aplicarFechaEfectivaTurno(false); }catch(e){}
            }catch(e){}

            // --- 5. HABILITAR CONTROLES Y APLICAR ESTADO INICIAL ---
            habilitarControles();
    ...
    if (btnGuardar) {
        ...
        var _guardandoInternasEnProceso = false;
        
        btnGuardar.addEventListener('click', function() {
            // FIX: Si estamos en la página de edición (recepcionesinternas/edicion),
            // el listener de recepcionesinternas_edicion.js es el que maneja el guardado.
            // Este listener solo debe ejecutarse en la página principal (index).
            if (window._esPaginaEdicion) {
                console.log('[RecepcionesInternas] Página de edición detectada, ignorando listener principal de guardado');
                return;
            }
            
            // FIX: Prevenir doble clic
            if (_guardandoInternasEnProceso) return;
            _guardandoInternasEnProceso = true;
            
            // FIX: Deshabilitar botón inmediatamente para evitar doble clic
            try {
                const _btnGuardarLocal = document.getElementById('btnGuardar');
                if (_btnGuardarLocal) {
                    _btnGuardarLocal.disabled = true;
                    _btnGuardarLocal.textContent = 'Guardando...';
                }
            } catch(e) {}
            ...
            // Construir endpoints (ruta amigable y fallback index.php)
            const action = recepciónId ? 'modificar' : 'guardar';
            if (recepciónId) payload.Id = recepciónId;
            const endpoints = [
                (window.APP_URL || window.BASE_URL) + '/recepcionesinternas/' + action,
                (window.BASE_URL || window.APP_URL) + '/index.php?url=recepcionesinternas/' + action
            ];

            function restaurarBtnGuardarInternas() {
                _guardandoInternasEnProceso = false;
            ...
            function trySave(i) {
                if (i >= endpoints.length) {
                    mostrarMensajeError('No se pudo contactar al servidor para guardar/modificar.', 'error');
                    restaurarBtnGuardarInternas();
                    return Promise.resolve();
                }
                const urlTry = endpoints[i];
                console.log('[RecepcionesInternas] intentar endpoint:', urlTry);
                try { console.log('[RecepcionesInternas] cookies:', document.cookie); } catch(e) { console.warn('No se pudo leer cookies', e); }
                return fetch(urlTry, {
                    method: 'POST',
                ...
                .then(text => {
                    try {
                        ...
                        if (res.success) {
                            ...
                            if (btnNuevo) btnNuevo.removeAttribute('disabled');
                            if (btnModificar) btnModificar.removeAttribute('disabled');
                            if (res.id) correlativoValeEl.setAttribute('data-id', res.id);
                            // Si fue creación, actualizar el correlativo mostrado al siguiente disponible
                            if (!recepciónId) {
                                const sv = (window.APP_URL || window.BASE_URL) + '/recepcionesinternas/siguienteVale';
                                fetch(sv).then(parseJSONorRedirect).then(rn => {
                                    if (rn && rn.success && rn.correlativo) {
                                        let num = rn.correlativo.toString().replace(/^VRI-/, '').padStart(6, '0');
                                        if (correlativoVale) correlativoVale.textContent = `VRI-${num}`;
                                    }
                                }).catch(()=>{});
                            }
                        ...
                        const snippet = (text || '').slice(0,800);
                        console.error('[RecepcionesInternas] respuesta no JSON en', urlTry, snippet);
                        if (snippet.indexOf('<!DOCTYPE') !== -1 || snippet.indexOf('Cerrar Sesión') !== -1) {
                            mostrarMensajeError('Sesión expirada o respuesta HTML recibida. Redirigiendo al login...', 'error');
                            setTimeout(() => { window.location.href = (window.APP_URL || '/') + '/login'; }, 900);
                            return Promise.resolve();
                        }
                        return trySave(i + 1);
                ...
                .catch(err => {
                    console.warn('[RecepcionesInternas] fetch error en', urlTry, err);
                    return trySave(i + 1);
                })
                .finally(function() {
                    // Si solo hay 1 endpoint y falló, restaurar botón
