...
(function() {
    ...
    console.log('[RecepcionesInternas - Edición] Script cargado');
    ...
    try {
        ...
        Object.defineProperty(window._dbg, 'getState', {
            ...
            enumerable: false,
            value: function() {
                try { return { datosGrilla: JSON.parse(JSON.stringify(datosGrilla)), editIndex: editIndex, updatePhase: updatePhase, valeIdActual: valeIdActual }; } catch(e) { return { datosGrilla: datosGrilla, editIndex: editIndex, updatePhase: updatePhase, valeIdActual: valeIdActual }; }
            }
        });
    } catch(e) { /* ignore */ }
    ...
    function initChoices() {
        ...
        function updateProductoChoiceWrap() {
            const productoChoicesContainer = document.querySelector('#productoEdit')?.closest('.choices');
        ...
        const productoEditSelect = document.getElementById('productoEdit');
            ...
            if (opt) {
                ...
                if (window.choicesInstances && window.choicesInstances['producto']) {
                    ...
                    setTimeout(()=> {
                        ...
                        // DESHABILITADO: try { if (typeof window.updateProductoChoiceWrap === 'function') window.updateProductoChoiceWrap(); } catch(e){}
                    }, 100);
    ...
    function setupBotones() {
        ...
        if (btnGuardar) {
            btnGuardar.addEventListener('click', guardarRecepcion);
        }
    ...
    function guardarRecepcion() {
        ...
        const url = (window.APP_URL || window.BASE_URL) + '/recepcionesinternas/guardar';
        fetch(url, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (valeIdActual) {
                    mostrarMensajeError('Recepción modificada correctamente.');
                } else {
                    mostrarMensajeError('Recepción guardada correctamente.');
                }
        ...
        .catch(error => {
            ...
            mostrarMensajeError('Error de conexión al guardar la recepción');
        });
    }

    function habilitarEdicion() {
        console.log('[Edición] Habilitando edición (excepto turno, area y codigo)');
        // Habilitar controles salvo los explícitamente protegidos
        const permitir = ['fecha','subarea','emisor','despachador','medioTransporte','verificador','producto','cantidad','comentarios'];
        permitir.forEach(id => {
            try {
                const el = document.getElementById(id);
                if (el) {
    ...
    window.cargarVale = function(valeId) {
        ...
        fetch((window.APP_URL || window.BASE_URL) + '/recepcionesinternas/obtenerVale?id=' + valeId)
            .then(response => response.json())
            .then(data => {
